---
title: 'Appendix: Racial_Support_Analysis'
...
getwd()
library(readr)
Main_data_AA <- read_csv("/cloud/project/ADVANCE_Manuscript_Appendix/Racial Support Analysis/Main_data_AA_All.csv", 
...
prepare_data <- function() {
  hdb_data <- data.frame(
    ...
    PRE = c(4.53, 4.04, 4.45, 3.96, 3.80, 4.00),
    POST = c(4.53, 3.73, 3.70, 3.67, 3.86, 4.17),
...
```{r , echo=FALSE}
#print(plot_clean)
ggsave(plot=plot_clean, filename="Racial_Support_dumbbell.png", 
       height=4.5, width=6.25, dpi=350, units="in")

```

### Interpretation of the Racial Support Theme Dumbbell Plot

The dumbbell plot shows the pre- and post-workshop scores for the Racial Support theme across different semesters from Fall 2019 to Fall 2023. Here’s a detailed interpretation of the results for each session and what they reveal about the workshops and participants:

1. **Fall 2023**:
   - **Pre-test Score**: 4.00
   - **Post-test Score**: 4.17
   - **Change**: +0.17
   - **Interpretation**: The scores increased by 0.17 points, suggesting that participants' perceptions of the Racial Support theme improved slightly after the workshop.

2. **Spring 2023**:
   - **Pre-test Score**: 3.80
   - **Post-test Score**: 3.86
   - **Change**: +0.06
   - **Interpretation**: There was a small increase of 0.06 points, indicating a minor positive change in participants' perceptions of the Racial Support theme.

3. **Fall 2022**:
   - **Pre-test Score**: 3.96
   - **Post-test Score**: 3.67
   - **Change**: -0.29
   - **Interpretation**: The decrease of 0.29 points suggests that the workshop led participants to adopt a more critical view of the Racial Support theme, possibly due to increased awareness of challenges or issues.

4. **Spring 2022**:
   - **Pre-test Score**: 4.45
   - **Post-test Score**: 3.70
   - **Change**: -0.75
   - **Interpretation**: A significant decrease of 0.75 points indicates that the workshop substantially influenced participants' views, encouraging a more critical perspective on the Racial Support theme.

5. **Fall 2021**:
   - **Pre-test Score**: 4.04
   - **Post-test Score**: 3.73
   - **Change**: -0.31
   - **Interpretation**: The decrease of 0.31 points suggests a moderate shift in participants' views. While the workshop led to some critical reflection, the overall perception remained moderately positive.

6. **Fall 2019**:
   - **Pre-test Score**: 4.53
   - **Post-test Score**: 4.53
   - **Change**: 0.00
   - **Interpretation**: The scores did not change, suggesting that the workshop had no impact on participants' perceptions of the Racial Support theme during this semester.

### Overall Interpretation and Workshop Impact

- **Overall Trend**: Most sessions show a decrease in post-test scores compared to pre-test scores, indicating that the workshops generally led participants to reassess their views on the Racial Support theme.
...
- **Critical Reflection**: The consistent decrease in scores suggests that the workshops were effective in promoting critical reflection among participants. By highlighting issues and challenges related to the Racial Support theme, the workshops encouraged participants to adopt a more nuanced and critical perspective.
- **Future Improvements**: The results highlight areas for potential improvement. For sessions with minimal changes (e.g., Fall 2023, Fall 2019), it may be beneficial to review and enhance the workshop content to ensure it effectively engages participants and prompts reflection.

### Conclusion

The dumbbell plot provides valuable insights into the impact of the Racial Support theme workshops across different semesters. While the overall trend indicates a decrease in scores, reflecting increased critical awareness, the variability suggests that further refinement of the workshops could enhance their effectiveness in promoting meaningful reflection and learning among participants.
...
# Graph by Filter
# Type = Race Support 
Main_data_AA_filter<-Main_data_AA[Main_data_AA$Type =="Race_Sup",]
#
##############
...
# Export PNG
ggsave(plot=SplitV, filename="Racial_Sup_V.png", 
       height=4.5, width=6.25, dpi=350, units="in")
...
### Interpretation of the Violin Plot for Racial Support

#### General Observation

The provided violin plot displays the pre-test (left, blue) and post-test (right, yellow) scores for the Racial Support theme across all semesters. The plot includes the distribution density, box plots with median and interquartile ranges, and individual data points for more granular insights.

#### Detailed Interpretation

1. **Pre-test Scores (Blue Violin)**:
    - **Distribution**: The pre-test scores are distributed with a peak around the median, suggesting most participants initially rated the Racial Support theme positively.
...
- **Shift in Perceptions**: The median post-test score is lower than the median pre-test score, indicating that participants' perceptions of the Racial Support theme became more critical after the workshop.
...
1. **Critical Reflection**: The shift in median scores suggests that the workshop effectively encouraged participants to critically reflect on the Racial Support theme. This critical reflection is a key goal of educational interventions aimed at raising awareness about complex issues.

2. **Diverse Impact**: The increased variability in post-test scores indicates that the workshop's impact varied among participants. This could be due to different levels of prior knowledge, engagement with the workshop material, or personal experiences related to the Racial Support theme.
...
race_sup_data <- data %>% filter(Type == "Race_Sup")

# View the first few rows of the filtered data
head(race_sup_data)

# Fit the Linear Mixed Model
lmm_race_sup <- lmer(Value ~ Test + (1 | Year) + (1 | Semester), data = race_sup_data)

# Summarize the model
summary(lmm_race_sup)
...
tidy_lmm <- tidy(lmm_race_sup, effects = "fixed")
...
### Interpretation of Fixed Effects for Racial Support Theme

1. **Intercept (Estimate = 3.7913)**:
   - **Estimate**: The estimated average score for the post-test is 3.79. This is the baseline against which other effects are compared.
   - **Std. Error**: The standard error of the intercept is 0.1144.
   - **t value**: The t-value for the intercept is 33.142, indicating the estimate is 33.142 standard errors away from zero.
   - **Pr(>|t|)**: The p-value is <2e-16, which is extremely small, indicating that the intercept is highly significant (p < 0.001).

2. **TestPre (Estimate = 0.2869)**:
   - **Estimate**: The estimated difference between the pre-test and post-test scores is 0.29. Since the estimate is positive, it indicates that, on average, the pre-test scores are 0.29 points higher than the post-test scores.
   - **Std. Error**: The standard error of the TestPre effect is 0.1533.
   - **t value**: The t-value for TestPre is 1.871, indicating the estimate is 1.871 standard errors away from zero.
   - **Pr(>|t|)**: The p-value is 0.0628, which is slightly above the 0.05 threshold, indicating that this effect is marginally significant.
...
- **Semester (Intercept)**: The variance of the random intercept for Semester is 0.000 with a standard deviation of 0.000. This indicates very little variability in the average scores between different semesters.
- **Residual**: The residual variance is 1.178 with a standard deviation of 1.085, representing the within-group variability.

### Description of the Workshop's Effects

Based on the results of the Linear Mixed Model, we can draw several conclusions about the effects of the workshop on participants' perceptions of the Racial Support theme:

1. **Baseline Perceptions (Intercept)**:
   - The estimated average score for the post-test is 3.79 (t = 33.142, p < 0.001). This suggests that, after the workshop, participants' perceptions of the Racial Support theme were moderately positive, with an average score close to 4 on a Likert scale.

2. **Impact of the Workshop (TestPre)**:
   - The estimated difference between the pre-test and post-test scores is 0.29 (t = 1.871, p = 0.0628). This positive coefficient indicates that, on average, participants' perceptions of the Racial Support theme were higher before attending the workshop compared to after. The marginally significant difference suggests that the workshop had some impact, leading to a reduction in scores.

3. **Interpretation of the Results**:
   - The reduction in post-test scores (by 0.29 points on average) suggests that the workshop may have led participants to reassess their initial perceptions of the Racial Support theme. This could be due to increased awareness of issues or challenges related to racial support that were highlighted during the workshop. Participants may have developed a more critical or nuanced understanding, resulting in lower post-test scores.

4. **Random Effects**:
   - There is no variability in the average scores between different years or semesters, as indicated by the random intercepts for Year and Semester. This suggests that the workshop's impact remains consistent across different time periods.
...
6. **Conclusion**:
   - In conclusion, while participants initially had a moderately positive perception of the Racial Support theme, the workshop appears to have led to a slight decrease in their ratings. This suggests that the workshop effectively challenged participants' pre-existing views, promoting a more critical perspective on the Racial Support theme. Future workshops should aim to balance raising awareness with providing solutions and support to address any concerns that arise.

### Summary of Key Points:

- **Intercept**: Post-test scores average around 3.79, indicating moderately positive perceptions after the workshop.
- **TestPre**: Pre-test scores are, on average, 0.29 points higher than post-test scores, indicating a marginally significant reduction in perceptions due to the workshop.
- **Significance**: The intercept is highly significant (p < 0.001), while the TestPre effect is marginally significant (p = 0.0628).
...
# Fall 2021
Main_data_AA_f21<-subset(Main_data_AA,Type=="Race_Sup"&Year=="2021"&Semester=="Fall")
...
# Climate Cohen's d Effect Size
# Spring 2022
Main_data_AA_s22<-subset(Main_data_AA,Type=="Race_Sup"&Year=="2022"&Semester=="Spring")
...
# Climate Cohen's d Effect Size 
# Fall 2022
Main_data_AA_f22<-subset(Main_data_AA,Type=="Race_Sup"&Year=="2022"&Semester=="Fall")
...
# Climate Cohen's d Effect Size 
# Spring 2023
Main_data_AA_s23<-subset(Main_data_AA,Type=="Race_Sup"&Year=="2023"&Semester=="Spring")
...
# Climate Cohen's d Effect Size 
# Fall 2023
Main_data_AA_f23<-subset(Main_data_AA,Type=="Race_Sup"&Year=="2023"&Semester=="Fall")
...
1. **Fall 2021 (Cohen’s d = 0.2538058, Small)**:
   - **Interpretation**: The workshop had a small effect size, indicating a limited impact on participants' perceptions. The change in views was present but not substantial.
...
2. **Spring 2022 (Cohen’s d = 0.8816042, Large)**:
   - **Interpretation**: The large effect size indicates a significant impact on participants' perceptions. The workshop was highly effective in changing views on the Racial Support theme.
...
3. **Fall 2022 (Cohen’s d = 0.2973751, Small)**:
   - **Interpretation**: The workshop had a small effect size, indicating a limited impact on participants' perceptions. The change in views was present but not substantial.
   - **Implications**: Similar to Fall 2021, the small effect size indicates a need to review the workshop's content and methodology to enhance its effectiveness. Additional support or interactive elements might be necessary to engage participants more deeply.

4. **Spring 2023 (Cohen’s d = -0.04925529, Negligible)**:
...
5. **Fall 2023 (Cohen’s d = -0.1787669, Negligible)**:
   - **Interpretation**: The negligible effect size suggests that the workshop had little to no impact on participants' perceptions. The pre-test and post-test scores were very similar.
   - **Implications**: The negligible effect size indicates a need for a thorough review of the workshop. Factors such as engagement, relevance of content, or external influences might have affected its impact. Adjustments are necessary to enhance the workshop's effectiveness for future participants.

6. **Fall 2019 (Cohen’s d = 0.4964378, Small)**:
   - **Interpretation**: The workshop had a small effect size, indicating a limited impact on participants' perceptions. The change in views was present but not substantial.
   - **Implications**: Similar to other small effect sizes, the limited impact suggests a need for reviewing and improving the workshop content and engagement strategies. 

### Summary

The Cohen's d results provide valuable insights into the effectiveness of the workshops across different semesters:

- **Large Effect Size (Spring 2022)**: This semester showed a significant impact, indicating that the workshop was highly effective. Understanding the factors contributing to this success can help replicate it in future sessions.
- **Small Effect Sizes (Fall 2021, Fall 2022, Fall 2019)**: These semesters showed a limited impact, suggesting that the workshops were somewhat effective but could benefit from further enhancements.
...
### Overall Summary of Key Takeaways from the Racial Support Analysis
...
- **Critical Reflection**: The consistent decrease in scores suggests that the workshops were effective in promoting critical reflection among participants, encouraging them to adopt a more nuanced and critical perspective on the Racial Support theme.
...
- **Pre-test Scores**: The pre-test scores are concentrated around the median, indicating a general consensus among participants with initially positive perceptions of the Racial Support theme.
...
- **Baseline Perceptions (Intercept)**: The estimated average post-test score is 3.79, indicating moderately positive perceptions after the workshop.
- **Impact of the Workshop (TestPre)**: The pre-test scores are, on average, 0.29 points higher than the post-test scores, indicating a marginally significant reduction in perceptions due to the workshop.
- **Statistical Significance**: The intercept is highly significant (p < 0.001), while the TestPre effect is marginally significant (p = 0.0628), indicating that the observed changes are only partially supported by statistical significance.
- **Random Effects**: No variability exists between different years or semesters, suggesting that while the overall trend shows a decrease in scores, it remains consistent across different semesters.
- **Implications**: The LMM results suggest that the workshops had some effect on participants' pre-existing views, promoting a more critical perspective on the Racial Support theme. Future workshops should aim to balance raising awareness with providing solutions and support to address any concerns that arise.

#### 4. Cohen's d Analysis
- **Spring 2022 (Large, d = 0.8816042)**: The workshop had a significant impact, indicating high effectiveness in changing participants' views.
- **Small Effect Sizes (Fall 2021, Fall 2022, Fall 2019)**: The workshops showed limited impact, suggesting that they were somewhat effective but could benefit from further enhancements.
- **Negligible Effect Sizes (Spring 2023, Fall 2023)**: The minimal impact indicates the need for a detailed review and substantial adjustments to the workshop.

### Conclusion
The analysis of the Racial Support theme across different semesters provides valuable insights into the effectiveness of the workshops:
